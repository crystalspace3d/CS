...
#include "include/iclouds.h" // Clouds interface
#include <iostream>
#include <string>
/**
...
class Simple : public csApplicationFramework, public csBaseEventHandler
{
 private:
  ...
  csRef<iEngine> engine;		/// A pointer to the 3D engine.
  csRef<iLoader> loader;		/// A pointer to the map loader plugin.
  csRef<iGraphics2D> g2d;		/// A pointer to the 3D renderer plugin.
  csRef<iGraphics3D> g3d;		/// A pointer to the 3D renderer plugin.
  csRef<iKeyboardDriver> kbd;		/// A pointer to the keyboard driver.
  csRef<iVirtualClock> vc;		/// A pointer to the virtual clock.
  csRef<iView> view;			/// A pointer to the view which contains the camera.
  iSector* room;			/// A pointer to the sector the camera will be in.
  //iFont*
  csRef<iFont> font;
 ...
 public:
  ...
  /**
    * Do a cloud rendering test!
    */
  void CloudTest (iSector* sector, int nbr, csVector3 pos, csVector3 box);
