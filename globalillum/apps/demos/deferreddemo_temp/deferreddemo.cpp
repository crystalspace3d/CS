...
bool DeferredDemo::SetupModules ()
{
  ...
  rm_default = csLoadPlugin<iRenderManager> (GetObjectRegistry (),
					     "crystalspace.rendermanager.unshadowed");
  if (!rm_default)
    return ReportError ("Failed to load unshadowed Render Manager!");

  engine->SetRenderManager (rm);
/*
  if (!ssdoManager.Initialize (GetObjectRegistry ()))
    return ReportError ("TODO text!");
*/
  ssdoResolution = config->GetStr ("RenderManager.Deferred.GlobalIllum.BufferResolution", "full");
...
bool DeferredDemo::SetupGui (bool reload)
{
  ...
  guiEnableSSDO   = static_cast<CEGUI::Checkbox*>(winMgr->getWindow ("EnableSSDO"));
  ...
  if (!guiRoot || 
      !guiDeferred || !guiForward || !guiEnableSSDO ||
...
void DeferredDemo::UpdateGui ()
{
  ...
  if (enableSSDO != guiEnableSSDO->isSelected ())
  {
    enableSSDO = !enableSSDO;
    ssdoManager.SetEnabled (enableSSDO);
  }
  ...
  if (!test) {
  ssdoManager.SetBlurEnabled (guiEnableBlur->isSelected ());
  ssdoManager.GetSSDOVariableAdd ("enable ambient occlusion")
    ->SetValue (guiEnableAO->isSelected () ? 1.0f : 0.0f);
  ssdoManager.GetSSDOVariableAdd ("enable indirect light")
    ->SetValue (guiEnableIndirectLight->isSelected () ? 1.0f : 0.0f);
  ssdoManager.GetSSDOVariableAdd ("occlusion strength")
    ->SetValue (occlusionStrength);
  ssdoManager.GetSSDOVariableAdd ("sample radius")
    ->SetValue (sampleRadius);
  ssdoManager.GetSSDOVariableAdd ("detail sample radius")
    ->SetValue (detailSampleRadius);
  ssdoManager.GetSSDOVariableAdd ("num passes")
    ->SetValue (aoPasses);
  ssdoManager.GetSSDOVariableAdd ("max occluder distance")
    ->SetValue (maxOccluderDistance);
  ssdoManager.GetSSDOVariableAdd ("self occlusion")
    ->SetValue (selfOcclusion);
  ssdoManager.GetSSDOVariableAdd ("bounce strength")
    ->SetValue (bounceStrength);
  ssdoManager.GetBlurVariableAdd ("ssao blur kernelsize")
    ->SetValue (blurKernelSize);
  ssdoManager.GetBlurVariableAdd ("ssao blur position threshold")
    ->SetValue (blurPositionThreshold);
  ssdoManager.GetBlurVariableAdd ("ssao blur normal threshold")
    ->SetValue (blurNormalThreshold);
  }
...
void DeferredDemo::Frame ()
{
  ...
  {
    test = false;
    ssdoManager.Initialize (GetObjectRegistry ());
  }
...
bool DeferredDemo::OnKeyboard (iEvent &event)
{
  ...
  if (eventtype == csKeyEventTypeDown)
  {
    ...
    {
      if (ssdoResolution.CompareNoCase ("full"))
        ssdoResolution = "half";
      else if (ssdoResolution.CompareNoCase ("half"))
        ssdoResolution = "quarter";
      else if (ssdoResolution.CompareNoCase ("quarter"))
        ssdoResolution = "full";

      ssdoManager.SetBufferResolution (ssdoResolution.GetDataSafe ());
      return true;
    }
    else if (code == 'x')
    {
      ...
      ssdoManager.SetNormalsAndDepthBufferResolution
	(depthNormalsResolution.GetDataSafe ());
      return true;
    }
    else if (code == 'l')
    {
