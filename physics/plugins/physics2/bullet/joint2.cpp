...
void csBulletJoint::Attach (CS::Physics::iPhysicalBody* body1, CS::Physics::iPhysicalBody* body2, bool forceUpdate)
{
  ...
  if (body2)
  {
    if (body1->GetObjectType () == CS::Physics::BODY_SOFT)
    {
      bodies[0] = body1;
      bodies[1] = body2;
    }
    ...
    {
      // Two rigid bodies.
      if (body2->GetObjectType () == CS::Physics::BODY_RIGID)
      {
        jointFlag &= ~JOINT_SOFT;
        bool static2 = (body2->QueryRigidBody ()->GetState () == CS::Physics::STATE_STATIC);
  ...
  else
  {
    ...
    if (body1->GetObjectType () == CS::Physics::BODY_RIGID)
      jointFlag &= ~JOINT_SOFT;
    else
    {
...
void csBulletJoint::AddBulletJoint ()
{
  ...
  else
  {
    ...
    if (type == SOFT_LINEAR_JOINT)
    {
      ...
      if (bodies[1]->GetObjectType () == CS::Physics::BODY_RIGID)
      {  
